!(function ($) {
    ...
    $.fn.facetwp_mapLayout = function (opts) {
        ...
        return this.each(function () {
            ...
            grid.addClass('_facetwp_map_grid_init');

            // setup loading
            grid.html('<div class="facetwp_map-loading" style=""></div>');
            ...
            var init_builder = function () {
                ...
                // remove loader
                grid.find('.facetwp_map-loading').remove();
            }

            var create_component = function (type, data) {

                var component = $('<div class="facetwp_map-component component-' + type + '" data-type="' + type + '" data-label="' + type + '"><div class="facetwp_map-component-toolbar"><span class="dashicons dashicons-admin-generic facetwp_map-component-edit"></span> ' + type + '<span class="dashicons dashicons-no remove-component right"></span></div></div>'),
            ...
            var remove_column = function (column) {
                ...
                var contents = column.find('.facetwp_map-component'),
                ...
                for (var r = 0; r < rows.length; r++) {
                    ...
                    for (var c = 0; c < columns.length; c++) {
                        ...
                        var column = $(columns[c]),
                            component_struct = [],
                            components = column.find('> .column-container > .facetwp_map-component');
            ...
            //  add element
            grid.on('click', '.facetwp_map-grid-element', function () {
                var clicked = $(this),
            ...
            grid.on('click', '.remove-component', function () {
                var component = $(this).closest('.facetwp_map-component');
                if (confirm('Really?')) {
            ...
            grid.on('click', '.remove-row', function () {
                var row = $(this).closest('.row'),
                    contents = row.find('.facetwp_map-component');
                if (contents.length) {
            ...
            var component_editor = function () {
                var clicked = $( this ),
                    ...
                    component_modal = $.facetwp_mapModal({
            ...
            grid.on('click', '.facetwp_map-component-edit', function () {
                var clicked = $(this),
                    component = clicked.closest('.facetwp_map-component'),
                    template_ele = $( '#' + grid.prop('id') + '-' + component.data('type') + '-tmpl' ),
                    template = Handlebars.compile( template_ele.html() );

                var component_modal = $.facetwp_mapModal({
            ...
            grid.on('click', '.add-component', function () {
                ...
                current_modal = $.facetwp_mapModal({
                    modal: 'facetwp_map_component',
                    title: 'Insert',
                    width : 550,
                    height: 445,
                    focus: true,
                ...
                $( current_modal.content ).on('click', '.facetwp_map-component-trigger', component_editor );
            ...
            function init_rows() {
                ...
                grid.sortable({
                    //handle : '.facetwp_map-row-toolbar',
                    //axis: "y",
                    connectWith: ".facetwp_map-control-layout",
                    delay: 150,
                    ...
                    helper: function (e, item) {
                        return $('<div class="facetwp_map-row-helper" style="width: 155px; height: 30px;"></div>');
                    },
                });

                sort_column_container = grid.find(".column-container").sortable({
                    //handle : '.facetwp_map-component-toolbar',
                    delay: 150,
                    forceHelperSize: false,
                    items: '.facetwp_map-component',
                    appendTo: "body",
                    connectWith: ".column-container",
                    cursorAt: {top: 35, left: 74},
                    zIndex: 1000000,
                    helper: function (e, item) {
                        return $('<div class="facetwp_map-component-helper" style="width: 155px; height: 30px;" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.data('label') + '</div>');
                    },
                    update: function () {
                ...
                grid.find('.row').each(function () {
                    ...
                    sort_column = row.find(".column").droppable({
                        accept: '.facetwp_map-facetwp_mapbuilder-layout-layout_grid-first_row-0-settings-conditions-mini',
                        greedy: true,
            ...
            /// components
            $(".facetwp_map-facetwp_mapbuilder-layout-layout_grid-first_row-0-settings-conditions-mini").draggable({
                cursor: "move",
                appendTo: "body",
                cursorAt: {top: 15},
                helper: function (event) {
                    var item = $(event.target);
                    return $('<div class="facetwp_map-component-helper" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.html() + '</div>');
                }
            });
    ...
    $(window).load(function () {
        $(document).on('facetwp_map.init', function () {
            $('.facetwp_map-control-layout').not('._facetwp_map_grid_init').facetwp_mapLayout();
        });
    });
