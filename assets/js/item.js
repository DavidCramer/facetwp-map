var facetwp_item_control_modal, facetwp_item_control_modal_handler;
(function ($) {

    var current_items = {}
    flush_current = false;

    facetwp_item_control_modal = function (obj) {
        ...
        $('#' + obj.modal + '_facetwpModal .facetwp-modal-footer [data-state="' + state + '"]').remove();

        return html;
    }

    facetwp_item_control_modal_handler = function (data, obj) {
    ...
    var save_current_edit = function (parent) {
        ...
        for (var i = 0; i < holders.length; i++) {
            ...
            var holder = $(holders[i]),
                ...
                items = holder.find('.facetwp-item'),
                configs = [];
    ...
    $(document).on('click', '.facetwp-item-edit', function () {
        var clicked = $(this),
            control = clicked.closest('.facetwp-control-item'),
            id = control.prop('id') + '-config',
            trigger = $('button[data-modal="' + id + '"]');


        current_items[ id ] = clicked.closest('.facetwp-item');
        flush_current = false;

        trigger.trigger('click');
    });

    $(document).on('click', '.facetwp-item-remove', function () {
        var clicked = $(this),
            control = clicked.closest('.facetwp-control-item'),
            id = control.prop('id') + '-config',
            trigger = $('button[data-modal="' + id + '"]'),
            item = clicked.closest('.facetwp-item');
    ...
    $(window).load(function () {
        $(document).on('facetwp.init', function () {
            $('.facetwp-control-item').not('._facetwp_item_init').each(function () {

                var holder = $(this),
                    input = $('#' + holder.prop('id') + '-control'),
                    data;
                if( holder.hasClass('_facetwp_item_init') ){
                    return;
                }
                holder.addClass('_facetwp_item_init');
                try {
