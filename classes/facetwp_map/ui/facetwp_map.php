...
/**
 * Core FWP_MAP abstract class.
 *
 * @package facetwp_map\ui
 * @author  David Cramer
 */
abstract class facetwp_map {
	 ...
	 * @param facetwp_map $parent Parent FWP_MAP Object
	 */
	...
	protected function actions() {
		...
		// init FWP_MAP headers
		add_action( $location, array( $this, 'enqueue_core' ) );

	}

	/**
	 * Register the FWP_MAP objects
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * @return object|\facetwp_map object instance
	 */
	...
	/**
	 * Register FWP_MAP depend js and css and call set assets
	 *
	 * @since 1.0.0
	 * @access protected
	 */
	protected function core_assets() {
		wp_register_script( 'facetwp_map', $this->url . 'assets/js/core' . FWP_MAP_ASSET_DEBUG . '.js' );
		wp_register_style( 'facetwp_map', $this->url . 'assets/css/core' . FWP_MAP_ASSET_DEBUG . '.css', array( 'dashicons' ) );
		wp_localize_script( 'facetwp_map', 'facetwp_mapApi', array( 'root' => esc_url_raw( rest_url() ), 'nonce' => wp_create_nonce( 'wp_rest' ) ) );
	...
	public function enqueue_core() {
		...
		// register facetwp_map core asset
		$this->core_assets();

		/**
		 * do object initilisation
		 *
		 * @param object current facetwp_map instance
		 */
		do_action( 'facetwp_map_admin_enqueue_scripts' . $this->type, $this );
	...
	/**
	 * Determin if a FWP_MAP object should be active for this screen
