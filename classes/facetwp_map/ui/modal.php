...
class modal extends panel {
	...
	public function is_submitted() {
		...
		return isset( $data[ 'facetwp_mapNonce_' . $this->id() ] ) && wp_verify_nonce( $data[ 'facetwp_mapNonce_' . $this->id() ], $this->id() );
	}
	...
	protected function set_active_styles() {
		...
		$style = 'h3#' . $this->id() . '_facetwp_mapModalLable { background: ' . $this->base_color() . '; }';
		$style .= '#' . $this->id() . '_facetwp_mapModal.facetwp_map-modal-wrap > .facetwp_map-modal-body:after {background: url(' . $this->url . 'assets/svg/loading.php?base_color=' . urlencode( str_replace( '#', '', $this->base_color() ) ) . ') no-repeat center center;}';
