 ...
 * @since 1.0.0
 */
class handlebars extends \facetwp_map\ui\control {
	...
	public function set_data( $data ) {
		if ( isset( $data[ $this->slug ] ) ) {
			...
			$this->data[ $this->id() ][ $this->slug ] = apply_filters( 'facetwp_map_' . $this->slug . '_sanitize_' . $this->type, $data[ $this->slug ], $this );
		}
	...
	public function render() {
		...
		$output = '<div class="facetwp_map-tab-canvas" data-app="' . esc_attr( $this->name() ) . '" ' . esc_attr( $this->build_attributes() ) . '></div>';
	...
	public function set_assets() {
		...
		$this->assets['script']['handlebars-control']  = array(
			'src'  => $this->url . 'assets/controls/handlebars/handlebars-control' . FWP_MAP_ASSET_DEBUG . '.js',
			'deps' => array( 'baldrick' ),
		);
