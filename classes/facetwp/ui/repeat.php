...
class repeat extends panel {
	...
	public function prepare_data() {
		$submit_data = facetwp()->request_vars( 'post' );
	...
	public function render() {
		...
		$output = '<div data-instance="{{json this}}" data-facetwp-template="' . esc_attr( $this->id() ) . '" ' . $this->build_attributes() . '>';
	...
	public function render_repeatable() {
		...
		$output = '<div class="facetwp-repeat">';
		$output .= $this->render_template();
		if ( ! empty( $this->child ) ) {
			$output .= $this->render_children();
		}

		$output .= '<button type="button" class="button button-small facetwp-remover"><span class="dashicons dashicons-no"></span></button> </div>';
	...
	public function render_repeatable_more() {
		...
		$this->instance  = 0;
		$output          = '<div class="repeatable-footer"><button type="button" class="button" data-facetwp-repeat="' . esc_attr( $this->id() ) . '">' . esc_html( $label ) . '</button></div>';
	...
	protected function set_active_styles() {
		...
		$style = '#' . $this->id() . ' .facetwp-repeat{ box-shadow: 1px 0 0 ' . $this->base_color() . ' inset, -37px 0 0 #f5f5f5 inset, -38px 0 0 #ddd inset, 0 2px 3px rgba(0, 0, 0, 0.05); };';
