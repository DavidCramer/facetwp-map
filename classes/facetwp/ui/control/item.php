 ...
 * @since 1.0.0
 */
class item extends \facetwp\ui\control {
	...
	public function setup() {
		...
		$this->struct['modal'] = array(
			'config' => array(
				'label'       => __( 'Add Item', 'facetwp' ),
				'description' => __( 'Setup Item', 'facetwp' ),
				'callback'    => 'wp_send_json_success',
				'attributes'  => array(
					'class' => 'page-title-action',
					'data-content' => 'facetwp_item_control_modal',
				),
				'height'      => 540,
				'width'       => 380,
				'config'      => array(
					'target'        => 'facetwp_item_control_modal_handler',
					'control'       => $this->id(),
				),
				'template'    => '{{json this}} <button type="button" class="facetwp-item-edit button button-small">' . esc_html__( 'Edit', 'facetwp' ) . '</button> | <button type="button" class="facetwp-item-remove button button-small">' . esc_html__( 'Remove', 'facetwp' ) . '</button>',
				'footer'      => array(
					'id'      => $this->slug . '_foot',
					'control' => array(
						'add_item' => array(
							'label'      => __( 'Add', 'facetwp' ),
							'type'       => 'button',
							'attributes' => array(
								'type' => 'submit',
								'data-state' => 'add',
		...
		add_action( 'facetwp_control_item_submit_' . $this->slug, $this->struct['modal']['config']['callback'], 100 );
	...
	public function handle_submit() {
		if ( isset( $this->child['config'] ) && $this->child['config']->is_submitted() ) {
			...
			do_action( 'facetwp_control_item_submit_' . $this->slug, $data );
		}
	}
	...
	public function render() {
		...
		$output .= '<div id="' . esc_attr( $this->id() ) . '" data-color="' . esc_attr( $this->base_color() ) . '" data-for="' . esc_attr( $this->id() ) . '-control" class="processing facetwp-control facetwp-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '">';
		$output .= '</div>';
	...
	public function label() {
		...
		if ( isset( $this->struct['label'] ) ) {
			$output .= '<label for="' . esc_attr( $this->id() ) . '-control" class="facetwp-add-row"><span class="facetwp-control-label">' . esc_html( $this->struct['label'] ) . '</span></label>';
		}
	...
	protected function set_active_styles() {
		...
		$style .= '.' . $this->id() . ' .facetwp-component-toolbar{background-color: ' . $this->base_color() . ' !important;}';
