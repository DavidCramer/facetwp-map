 ...
 * @since 1.0.0
 */
class layout extends \facetwp\ui\control {
	...
	public function render() {
		...
		$output = '<div id="' . esc_attr( $this->id() ) . '" data-color="' . esc_attr( $this->base_color() ) . '" data-for="' . esc_attr( $this->id() ) . '-control" class="facetwp-control facetwp-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '"></div>';
		$output .= $this->label();
		$output .= $this->input();
	...
	public function component_templates() {
		...
		foreach ( $this->modals as $modal ) {
			...
			if ( isset( $modal->struct['preview'] ) ) {
				$template = facetwp()->add( 'control', $modal->slug, array(
					'type'     => 'template',
	...
	public function modal_template() {
		...
		foreach ( $this->modals as $modal ) {
			...
			$output .= '<button type="button" class="button facetwp-component-trigger" style="margin:12px 0 0 12px;" data-label="' . esc_attr( $modal->attributes['data-title'] ) . '" data-type="' . $modal->slug . '" ' . $setup . ' data-id="' . esc_attr( $modal->id() ) . '">' . $label . '</button> ';
