 ...
 * @since 1.0.0
 */
class toggle extends \facetwp\ui\control {
	...
	public function sdescription() {
		...
		if ( isset( $this->struct['description'] ) ) {
			$output .= '<span class="facetwp-toggle-description">' . esc_html( $this->struct['description'] ) . '</span>';
		}
	...
	protected function set_active_styles() {
		...
		if ( ! empty( $this->struct['base_color'] ) ) {
			$style .= '.' . $this->id() . ' > .facetwp-control-input > .switch.active {background: ' . $this->struct['base_color'] . ';}';
		}

		if ( ! empty( $this->struct['off_color'] ) ) {
			$style .= '.' . $this->id() . '> .facetwp-control-input > .switch { background: ' . $this->struct['off_color'] . ';}';
		}
