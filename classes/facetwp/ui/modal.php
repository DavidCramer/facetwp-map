...
class modal extends panel {
	...
	public function is_submitted() {
		...
		return isset( $data[ 'facetwpNonce_' . $this->id() ] ) && wp_verify_nonce( $data[ 'facetwpNonce_' . $this->id() ], $this->id() );
	}
	...
	public function modal_template() {
		...
		$output = wp_nonce_field( $this->id(), 'facetwpNonce_' . $this->id(), true, false );
	...
	protected function set_active_styles() {
		...
		$style = 'h3#' . $this->id() . '_facetwpModalLable { background: ' . $this->base_color() . '; }';
		$style .= '#' . $this->id() . '_facetwpModal.facetwp-modal-wrap > .facetwp-modal-body:after {background: url(' . $this->url . 'assets/svg/loading.php?base_color=' . urlencode( str_replace( '#', '', $this->base_color() ) ) . ') no-repeat center center;}';
