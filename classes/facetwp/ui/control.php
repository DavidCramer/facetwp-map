...
/**
 * Base fwp_map Control class.
 *
 * @since       1.0.0
 */
class control extends \facetwp\data\data {
	...
	public function render() {
		...
		$output = '<div id="' . esc_attr( $this->id() ) . '" class="facetwp-control facetwp-control-' . esc_attr( $this->type ) . ' ' . esc_attr( $this->id() ) . '">';

		$output .= $this->label();
		$output .= '<div class="facetwp-control-input">';
		$output .= $this->input();
		$output .= '</div>';
	...
	public function label() {
		...
		if ( isset( $this->struct['label'] ) ) {
			$output .= '<label for="' . esc_attr( $this->id() ) . '-control"><span class="facetwp-control-label">' . esc_html( $this->struct['label'] ) . '</span></label>';
		}
	...
	public function description() {
		...
		if ( isset( $this->struct['description'] ) ) {
			$output .= '<span class="facetwp-control-description">' . esc_html( $this->struct['description'] ) . '</span>';
		}
